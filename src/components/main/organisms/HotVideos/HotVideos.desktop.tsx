  ...
  return (
    <div className="mt-13 flex px-2">
      <div className="flex basis-1/3 flex-col items-center gap-y-4">
        <p className="text-black-normal ml-9 self-start text-2xl font-extrabold">Top1</p>
        <div className="relative">
          <img src={slicedVideos[0]?.thumbnail} alt="썸네일" className="h-42 w-62 object-cover" />
          <div className="absolute top-0 left-0 z-10">
            {top?.gradeStatus === 'COMPLETED' && typeof top.gradePercent === 'number' ? (
              <FactBadge
              ...
              />
            ) : top?.gradeStatus === 'PENDING' || top?.gradeStatus === 'FAILED' ? (
              <FactBadge percent={0} width={90} height={110} type={percentToValidity(0)} />
            ) : null}
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <a href={slicedVideos[0]?.link} target="_blank" rel="noopener noreferrer">
